...
\begin{problem}
	Find the limit, if it exists:
	\[ \lim_{z \to 0} \left( \cos z \right)^{\frac{1}{z^3}} \]
\end{problem}
...
\begin{heading}
	Discontinuity of Complex Functions
\end{heading}
\vspace{10pt}
\begin{definition}[Discontinuity of Complex Functions]
	A function \( f: D \to \C \) is said to be discontinuous at a point \( z_0 \in D \) if it is not continuous at \( z_0 \). In other words, \( f \) is discontinuous at \( z_0 \) if
	\[
		\lim_{z \to z_0} f(z) \ne f(z_0),
	\]
	or if the limit does not exist, or if \( f(z_0) \) is not defined.
