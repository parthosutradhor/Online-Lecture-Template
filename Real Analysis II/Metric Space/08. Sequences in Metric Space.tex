...
\begin{heading}
    Examples of Sequences in Metric Spaces
\end{heading}
...
Let
\[
X=\left\{0\right\}\cup\left\{\frac1n : n\in\mathbb N\right\}.
\]
Define two metrics on \(X\) by
\[
d_1(x,y)=|x-y|,
\qquad
d_2(x,y)=
...
x_n=\frac1n.
\]
Show that \((x_n)\) converges in the metric space \((X,d_1)\) but does not converge in the metric space \((X,d_2)\).
\end{problem}
\clearpage


\begin{problem}
Let \(X=C[0,1]\), the space of all continuous real-valued functions on \([0,1]\). Define two metrics on \(X\) by
\[
...
f_n(x)=x^n.
\]
Show that \((f_n)\) converges in the metric space \((X,d_1)\) but does not converge in the metric space \((X,d_2)\).
\end{problem}
\clearpage
