...
\begin{definition}[Subsequence]
    A \textit{subsequence} of a sequence $\{x_n\}$ is a sequence of the form $\{x_{n_k}\}$ where $\{n_k\}$ is a strictly increasing sequence of natural numbers.
\end{definition}

\vspace{200pt}

\begin{definition}[Limit Point]
    A point $x$ is a \textit{limit point} of a set $A$ if every neighborhood of $x$ contains at least one point of $A$ different from $x$ itself.
\end{definition}

\clearpage

\begin{theorem}[In metric spaces, One Point imply infinitely many points]
    If $x$ is a limit point of a infinite set $A$ in a metric space, then there are infinitely many points of $A$ in every neighborhood of $x$.
\end{theorem}
...
\begin{definition}[Bolzano-Weierstrass Property]
    A subset $K$ of a metric space is said to have the \textit{Bolzano-Weierstrass property} if every infinite subset of $K$ has a limit point in $K$.
\end{definition}
