...
\newcommand*{\DocDate}{June 23, 2026}
...
\begin{tcolorbox}[colback=ProbBack, colframe=ProbMain]
	Note: The relation $\sim$ between two sets is an equivalence relation, since it is reflexive, symmetric, and transitive.
\end{tcolorbox}
...
\begin{problem}
	Show that the set of natural numbers \(\mathbb{N}\) and the set of perfect squares \(\mathbb{S} = \{1, 4, 9, 16, 25, \ldots\}\) have the same cardinality.
\end{problem}
...
\vspace{15pt}
\begin{tcolorbox}[colback=ProbBack, colframe=ProbMain]
	Note: An infinite set which is countable is often referred to as \textit{countably infinite} or \textit{denumerable}. For this type of set we can list its elements in a sequence (like the natural numbers).
\end{tcolorbox}
...
\begin{definition}[Ordering of Cardinal Numbers (Using Injections)]
...
\begin{definition}[Ordering of Cardinal Numbers (Using Surjections)]
	For any sets $A$ and $B$:
	\begin{enumerate}
		\item $\card(A) \le \card(B)$ if and only if there exists a surjective function $f : B \to A$.
		\item $\card(A) = \card(B)$ if and only if there exists a bijective function $f : A \to B$.
		\item $\card(A) < \card(B)$ if and only if $\card(A) \le \card(B)$ but $A \not\sim B$.
	\end{enumerate}
\end{definition}
...
\begin{theorem}[Schröder–Bernstein Theorem]
	Let \(A\) and \(B\) be two sets. If there exist injective functions \(f: A \to B\) and \(g: B \to A\), then there exists a bijective function \(h: A \to B\). In other words, if \(\card(A) \leq \card(B)\) and \(\card(B) \leq \card(A)\), then \(\card(A) = \card(B)\).	
\end{theorem}

\clearpage

\begin{theorem}[Schröder–Bernstein Theorem (Using Surjections)]
	Let \(A\) and \(B\) be two sets. If there exist surjective functions \(f: A \to B\) and \(g: B \to A\), then there exists a bijective function \(h: A \to B\). In other words, if \(\card(B) \leq \card(A)\) and \(\card(A) \leq \card(B)\), then \(\card(A) = \card(B)\).	
\end{theorem}
