...
A function $f$ is continuous on an interval $I$ if for every $c \in I$ and for every $\epsilon > 0$, there exists a $\delta > 0$ such that for all $x \in I$, if $|x - c| < \delta$, then $|f(x) - f(c)| < \epsilon$. Here $\delta$ may depend on both $\epsilon$ and $c$.
\end{definition}
...
A function $f$ is uniformly continuous on an interval $I$ if for every $\epsilon > 0$, there exists a $\delta > 0$ such that for all $x, y$ in the domain of $f$, if $|x - y| < \delta$, then $|f(x) - f(y)| < \epsilon$.
\end{definition}
...
\begin{problem}
    Show that if $f$ is increasing on $[a,b]$, then $f$ is Riemann integrable on $[a,b]$.
\end{problem}
